    ...
    def apply(self, df: pd.DataFrame, outlier_mask: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        ...
        if outlier_mask is None:
            raise HandlingException(
                error_code="HEX001",
                method=self.__class__.__name__,
                suggestion=f"The '{self.__class__.__name__}' method requires an outlier mask generated by a detector."
            )
        ...
        for col in self.group_by:
            if col not in df.columns:
                raise HandlingException(
                    error_code="HEX004",
                    method=self.__class__.__name__,
                    suggestion=f"Group column '{col}' is missing from the input DataFrame."
                )
        ...
        for col in self.columns:
            if col in outlier_mask.columns and col not in self.group_by:
                ...
                replacements = temp_series.groupby(df_clean[self.group_by]).transform(self.agg_func)    # type: ignore
